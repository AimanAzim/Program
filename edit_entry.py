...
def save_to_database():
    try:
        ...
        cursor.execute("SELECT SrNo FROM rent_rec WHERE SrNo = ?", (sr_no,))
        existing_sr_no = cursor.fetchone()

        if existing_sr_no:
            
            cursor.execute("UPDATE rent_rec SET Name=?, Rent=?, FromDate=?, ToDate=?, CurrentLightUnit=?, PreviousLightUnit=?, TotalUnits=?, LightBill=?, DueAmount=?, Total=?, Remarks=? WHERE SrNo = ?",
                           (name, rent, from_date, to_date, current_units, previous_units, total_units, light_bill, due_amount, total, remarks, sr_no))
        else:
